...
export default function createSpecialExportRouter({ prisma, canAccess, teamWhere }) {
  ...
  async function datev(req, res) {
    if (!canAccess(req, 'datev_export', 'P')) return res.status(403).render('error', { message: (res.locals?.t ? res.locals.t('no_export_permission') : 'Keine Export-Berechtigung') });
  ...
  async function serienbriefSteuerdatei(req, res) {
    if (!canAccess(req, 'adressen', 'P')) return res.status(403).render('error', { message: (res.locals?.t ? res.locals.t('no_export_permission') : 'Keine Export-Berechtigung') });
  ...
  async function verkaufSteuerdatei(req, res) {
    if (!canAccess(req, 'verkauf', 'P')) return res.status(403).render('error', { message: (res.locals?.t ? res.locals.t('no_export_permission') : 'Keine Export-Berechtigung') });
  ...
  router.get('/Serienbrief', async (req, res) => {
    if (!canAccess(req, 'serienbrief', 'P')) return res.status(403).render('error', { message: (res.locals?.t ? res.locals.t('no_export_permission') : 'Keine Export-Berechtigung') });
